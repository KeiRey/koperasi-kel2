                        ...
                        <Menu.Items className='absolute rounded-lg w-[500px] border md:right-0 z-[100] bg-white dark:bg-gray-950 dark:border-gray-900'>
                            ...
                            <div className='p-4'>
                                <div className='flex flex-col gap-2 items-start h-60 overflow-y-auto'>
                                    {/* If data is empty */}
                                    {lowStockCount.length === 0 ? (<div className='text-sm text-gray-500 dark:text-gray-400'>Tidak ada notifikasi</div>) : (
                                        lowStockCount.map((data, i) => (
                                            <div className='flex items-center justify-between w-full p-2 rounded-lg border hover:bg-gray-200 dark:hover:bg-gray-900 ' key={i}>
                                                <div className='flex items-center w-full justify-between gap-4'>
                                                    <div className='flex flex-1 items-center gap-3'>
                                                        <img src={data.image} alt="" className='w-12' />
                                                        <span className='text-gray-500 dark:text-gray-400'>{data.title}</span>
                                                    </div>
                                                    <span className='text-gray-500 dark:text-gray-400'>Sisa : {data.stock}</span>
                                                    {/* <div>
                                                        <div className='font-semibold text-sm text-gray-700 dark:text-gray-200 '>
                                                            {lowStockCount.user} <sup className='text-xs font-mono text-gray-400 ml-1'>{lowStockCount.time}</sup>
                                                        </div>
